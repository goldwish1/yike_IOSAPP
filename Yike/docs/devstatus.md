...
14. [ ] 导航机制弹窗关闭问题修复
   ...
   - [ ] 调整操作顺序
     - [ ] 确保在导航前完成所有资源清理
     - [ ] 使用异步机制确保资源清理完毕后再导航
     - [ ] 添加状态检查避免在资源清理未完成时执行导航
   - [ ] 分离关注点实施方案
     - [ ] 创建纯音频播放器组件(AudioPlayerCore)
       - [ ] 设计简洁API，所有异步操作提供回调
       - [ ] 只关注音频播放本身，不处理缓存或网络
       - [ ] 明确反映播放状态，提供精确控制
     - [ ] 实现资源管理器(ResourceManager)
       ...
       - [ ] 处理网络请求和本地存储
       - [ ] 提供清晰的异步操作完成回调
     - [ ] 开发播放控制协调器(PlaybackCoordinator)
       - [ ] 协调音频播放器和资源管理器
       - [ ] 管理复杂操作流程
       - [ ] 整合异步操作的时序控制
     - [ ] 改进导航控制器(NavigationController)
       - [ ] 专注导航流程，不直接处理资源
       - [ ] 提供资源清理完成后的导航机制
       - [ ] 防止导航操作被异步操作阻塞
   - [ ] 重构现有代码
     - [ ] 重构PlaybackControlViewModel
     - [ ] 优化PlayerView中的确认按钮处理逻辑
     - [ ] 简化ApiVoicePlaybackManager和LocalVoicePlaybackManager
     - [ ] 添加超时保护机制
   - [ ] 测试与验证
     - [ ] 测试弹窗确认后的页面关闭
     - [ ] 验证资源是否正确清理
